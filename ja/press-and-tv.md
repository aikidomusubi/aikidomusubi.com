---
layout: video
title: メディア掲載
description: "インタビュー・取材・テレビ出演の記録です。"
permalink: /ja/press-and-tv/
# The videos page lived here for years; the stub keeps those links working.
redirect_from:
  - /ja/videos/
lang: ja
i18n-ref: press-and-tv-Rm4zW8dL
last_modified: 2026-08-18
---

# メディア掲載

インタビューや取材、テレビ出演の記録です。動画は[YouTubeチャンネル](https://www.youtube.com/user/{{ site.youtube_username }}/){:target="_blank" rel="noopener"}でも公開しています。特に記載のない限り、映像の権利は各制作者に帰属します。

